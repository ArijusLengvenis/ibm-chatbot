...
function sendMessage() {
    ...
    document
        ...
        .appendChild(generateUserMessageBlock(message))
    id++
        ...
        .then((data) => {
            ...
            if (data.length > 0) {
                ...
                if (data[0].documentId) {
                    chatBox.replaceChild(
                        generateChatbotMessageBlock(data),
                        chatBox.querySelector(`#message${id}`)
                    )
                } else {
                    chatBox.replaceChild(
                        chatbotSays(data[0].text),
                        chatBox.querySelector(`#message${id}`)
                    )
                }
                id++
...
function urlProcessing(data) {
    ...
    data.forEach((message, i) => {
        const urlMatches = message.text.match(URLR)
...
function generateChatbotMessageBlock(messages) {
    ...
    showMessages.forEach((message, i) => {
        ...
        if (
            i == 0 &&
            message !==
                "I searched my knowledge base, but did not find anything related to your query."
        ) {
        ...
        if (
            ...
            thumbsUp.setAttribute("id", `p${message.documentId}`)
            ...
            thumbsDown.setAttribute("id", `p${message.documentId}`)
            thumbsDown.setAttribute(
                "onClick",
                `rateAnswer(${id}, ${i}, ${false})`
            )
...
function loadMore(messageId) {
    ...
    messagesHere.forEach((message, i) => {
        ...
        thumbsDown.setAttribute(
            "onClick",
            `rateAnswer(${messageId}, ${i + 2}, ${false})`
        )
        ratingsDiv.appendChild(thumbsUp)
        ratingsDiv.appendChild(thumbsDown)
        messageWrapper.appendChild(ratingsDiv)
...
function rateAnswer(messageId, answerId, relevant) {
    ...
    let messageDiv = document.querySelector(
        `#message${messageId} #ratingId${answerId}`
    )
    ...
    if (message.oldRelevant === undefined) message.oldRelevant = null
    console.log({
...
function initChatbotSession() {
    ...
    fetch("/createsession", {
        ...
        .then((data) => {
            ...
            chatBox.replaceChild(
                chatbotSays(
                    "Hi I'm IBM's Chatbot, here to answer any of your questions about IBM Cloud and Cloud for Finance!"
                ),
                chatBox.querySelector(`#message${id}`)
            )
            id++
        ...
        .catch((err) => {
            ...
            chatBox.replaceChild(
                chatbotSays(
                    "Sorry, an error occured while connecting to IBM. Try refreshing the page or contacting support!"
                ),
                chatBox.querySelector(`#message${id}`)
            )
            id++
